...
(defclass db/postgresql (db-backend)
  ...
  (:default-initargs
   ...
   :host "localhost"
   :port 5432
   :sql-insert-log-statement "insert into log(version, colour, report, signature, client_signature, unixtimestamp) values ('~A', '~A', '~A', '~A', '~A', '~A', round(extract(epoch from now())));"))
